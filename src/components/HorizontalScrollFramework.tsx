...
/** 
 * THE STRATEGIC HORIZON (FIXED)
 * A cinematic horizontal scrolling experience for institutional strategy.
 * Uses GSAP ScrollTrigger for maximum stability in pinned sections.
 */
...
export default function HorizontalScrollFramework() {
  ...
  useLayoutEffect(() => {
    ...
    const amountToScroll = slideContainerRef.current.offsetWidth - window.innerWidth;
    ...
    const ctx = gsap.context(() => {
      gsap.to(slideContainerRef.current, {
        ...
        scrollTrigger: {
          ...
          start: "top top",
          end: () => `+=${slideContainerRef.current?.offsetWidth || 0}`,
          pin: true,
          scrub: 1,
  ...
  return (
    ...
    {/* PC Section with GSAP Pinning */}
    <section ref={containerRef} className="relative hidden w-full bg-surface-lowest md:block overflow-hidden">
      ...
      <div ref={slideContainerRef} className="flex w-[400vw] h-screen relative items-center z-10 will-change-transform">
        
        {/* PANEL 1: THE MANIFESTO / HORIZON */}
        <div className="horizontal-panel h-screen w-screen flex flex-col justify-center px-10 md:px-24 lg:px-40 relative">
        ...
        {/* STRATEGY MODULES (2nd, 3rd, 4th panels) */}
        {strategySegments.map((segment, idx) => (
          <div key={idx} className="horizontal-panel h-screen w-screen flex items-center justify-center px-10 md:px-20 relative">
